...
ImageTexture::ImageTexture(const float ka_r, const float ka_g, const float ka_b) {
	// Create a 1x1 texture with the given color Kd.
	imageWidth = 1;
	imageHeight = 1;
	numChannels = 3;
	textureObj = 0;
	glm::vec3 Kd = glm::vec3(ka_r, ka_g, ka_b);

	glGenTextures(1, &textureObj);
	glBindTexture(GL_TEXTURE_2D, textureObj);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, imageWidth, imageHeight,
		0, GL_RGB, GL_FLOAT, &Kd);
