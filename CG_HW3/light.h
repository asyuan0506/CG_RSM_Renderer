...
public:
	...
	void SetPosition(const glm::vec3 p) { position = p; } // For UI manager use.
...
public:
	...
	SpotLight() {
		...
		cutoffStart_ = 15.0f; // in degree
		totalWidth_ = 30.0f;  // in degree
		// -------------------------------------------------------
		CreateVisGeometry();
	}
	SpotLight(const glm::vec3 p, const glm::vec3 I, const glm::vec3 D, const float cutoffDeg, const float totalWidthDeg) {
		...
		// Add your initialization code here.
		direction_ = D;
		cutoffStart_ = cutoffDeg;
		totalWidth_ = totalWidthDeg; 
		// -------------------------------------------------------
		CreateVisGeometry();
	}
	...
	void	  SetDirection(const glm::vec3 dir) { direction_ = dir; } // For UI manager use.
	float     GetCutoffStart()  const { return cutoffStart_; }
	float     GetTotalWidth()   const { return totalWidth_; }
	// -------------------------------------------------------
...
private:
	...
	// Add your methods or data here.
	glm::vec3 direction_;
	float cutoffStart_; // in degree
	float totalWidth_;  // in degree
	// -------------------------------------------------------
};
